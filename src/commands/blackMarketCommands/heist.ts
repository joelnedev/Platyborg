import { platyborg, blackMarket, randomNumber, replaceReplies } from "../../util/index.js";
import { CommandInteraction, GuildMember, MessageEmbed } from "discord.js";
export const execute = async (interaction: CommandInteraction) => {

	// Set variables
	const member = await platyborg.PFC.members.fetch(interaction.user.id);
	const config = platyborg.config.economy.heist;
	...
	const winReplies = await replaceReplies(platyborg.config.replies.crime.win, add);
	const failReplies = await replaceReplies(platyborg.config.replies.crime.fail, remove);
