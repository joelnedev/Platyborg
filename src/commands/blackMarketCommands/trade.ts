import { blackMarket, item, platyborg } from "../../util/index.js";
...
export const execute = async (interaction: CommandInteraction, command: any) => {
	...
	const target = await blackMarket.getUser(command.args.target?.id);
	...
	if (!item) {
		...
		const m = (await (await platyborg.users.fetch(user.id)).dmChannel?.send({ content: `${interaction.user?.tag} wants to trade you ${item.name} for ${targetItem!.name}. Do you accept the transaction? Accept or decline within 30 seconds.`, components: [buttons] }));
