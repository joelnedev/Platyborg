import { Bot, GlobalCommand, Interaction } from "../util/exports.js";
import { GuildEmoji, Snowflake, User } from "discord.js";
export default new GlobalCommand({
	...
	async execute(interaction: Interaction) {
		...
		const subcommand = interaction.args[0].options?.[0]; // @ts-expect-error
		...
		switch (subcommand?.name) {
			...
			case "sell":
				...
				if (subcommand?.options?.find((arg: any) => arg.name === "user")?.value) {
					command.args.target = await Vagan.users.fetch(subcommand?.options?.[0].value);
				}
				break;
			case "trade":
		...
		try {
			await require(`./blackMarketCommands/${command.name}.js`).execute(interaction, command);
		} catch (error) {
